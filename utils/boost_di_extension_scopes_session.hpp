...
class shared {
public:
  template <class, class T> class scope {
  public:
...
class shared_config : public di::config {
  ...
  template <class T> static auto type_id() {
...
public:
  ...
  template <class T> struct scope_traits<T &> { using type = detail::shared; };
  ...
  template <class T> auto &data() { return data_[type_id<T>()]; }
