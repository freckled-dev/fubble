...
// TODO rename not google specific no more
...
public:
  ...
  void set_source(std::shared_ptr<rtc::video_source> source);
  std::shared_ptr<rtc::video_source> get_source() const;
  void set_surface(QAbstractVideoSurface *surface);
  QAbstractVideoSurface *get_surface() const;

protected:
  void on_frame(const rtc::video_frame &frame);
  ...
  utils::signal::scoped_connection connection_on_frame;
};
} // namespace client::ui
