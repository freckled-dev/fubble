...
public:
  loopback_audio_impl(rtc::factory &rtc_factory,
    ...
    std::shared_ptr<rtc::audio_track> sending_audio_track = audio_source;
  ...
  void on_audio_track(rtc::track_ptr track) {
    ...
    audio_track = std::dynamic_pointer_cast<rtc::audio_track>(track);
...
loopback_audio::create(rtc::factory &rtc_factory,
                       std::shared_ptr<rtc::audio_track> audio_source,
                       std::shared_ptr<boost::executor> defering_executor) {
...
std::shared_ptr<rtc::audio_track> loopback_audio_noop_if_disabled::get_track() {
  if (!delegate)
    return nullptr;
