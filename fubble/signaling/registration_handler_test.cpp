...
struct RegistrationHandler : testing::Test {
  ...
  signaling::device::creator creator{executor};
  signaling::registration_handler handler{creator};
};

struct mock_connection final : signaling::connection {
  ~mock_connection() final { on_closed(); }
  boost::optional<signaling::offer> offer;
  void send_offer(const signaling::offer &send) final {
    EXPECT_FALSE(offer);
    offer = send;
  }
  std::vector<signaling::ice_candidate> candidates;
...
add_connection(signaling::registration_handler &handler,
...
TEST_F(RegistrationHandler, OfferSend) {
  ...
  const signaling::offer offer{"sdp"};
...
TEST_F(RegistrationHandler, AnswerSend) {
  const auto offering = add_offering_connection(handler);
  const auto answering = add_connection(handler);
  const signaling::answer answer{"sdp"};
...
TEST_F(RegistrationHandler, IceCandidateToAnswer) {
  ...
  const auto answering = add_connection(handler);
  signaling::ice_candidate candidate{0, "mline", "candidate"};
...
TEST_F(RegistrationHandler, ReconnectResend) {
  ...
  answering->on_ice_candidate(
      signaling::ice_candidate{0, "mline", "candidate"});
  const auto replacing = add_connection(handler, offering_token);
